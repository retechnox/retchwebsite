...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_internship'])) {
    ...
    // Handle Image Upload for Registration Display
    ...
    if (move_uploaded_file($_FILES['qr_code']['tmp_name'], $qrCodePath)) {
        ...
        $stmt = $pdo->prepare("INSERT INTO internships (title, description, gpay_link, qr_code_image, amount, whatsapp_message, display_image) VALUES (?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([$title, $description, $gpay_link, $qrCodePathForDB, $amount, $whatsapp_message, $displayImagePathForDB]);
        echo "<div class='alert alert-success'>Internship added successfully!</div>";
    } else {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['approve'])) {
    ...
    $stmt = $pdo->prepare("UPDATE internship_registrations SET status = 'approved' WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['delete'])) {
    $registration_id = $_POST['registration_id'];
    $stmt = $pdo->prepare("DELETE FROM internship_registrations WHERE id = ? AND status = 'pending'");
    $stmt->execute([$registration_id]);
...
$registrations = $pdo->query("SELECT r.*, i.title, i.whatsapp_message FROM internship_registrations r JOIN internships i ON r.internship_id = i.id WHERE r.status = 'pending'")->fetchAll();
?>
...
<body>
    <div class="container mt-5">
        ...
        <form action="admin_internship.php" method="post" enctype="multipart/form-data">
            <!-- Display Image Upload Field -->
            <div class="form-group">
                <label for="display_image">Image for Registration Display</label>
            ...
            <div class="form-group">
                ...
                <p>upi://pay?pa=techtopservice1@oksbi&pn=Tech%Top&am=100.00&cu=INR</p>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control" id="whatsapp_message" name="whatsapp_message" rows="2"
                    required></textarea>
            </div>
            <button type="submit" name="add_internship" class="btn btn-primary">Add Internship</button>
        </form>
            ...
            <ul class="list-group">
                <?php foreach ($registrations as $registration): ?>
                    <li class="list-group-item">
                        <strong>Internship:</strong> <?php echo htmlspecialchars($registration['title']); ?><br>
                        ...
                        <strong>College Name:</strong> <?php echo htmlspecialchars($registration['college_name']); ?><br>
                        ...
                        <strong>Remarks:</strong> <?php echo htmlspecialchars($registration['screenshot_image']); ?><br>
                        ...
                        <!-- Approve Button -->
                        <form action="admin_internship.php" method="post" style="display:inline-block;">
                        ...
                        <?php if ($registration['phone_number']): ?>
                            <a href="https://wa.me/<?php echo htmlspecialchars($registration['phone_number']); ?>?text=<?php echo urlencode($registration['whatsapp_message']); ?>"
